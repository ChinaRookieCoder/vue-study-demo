...
export const browser = ((n) => {
  const u = n.userAgent; const app = n.appVersion
  return {
...
export const extend = function() {
  let options; let name; let src; let copy; let copyIsArray; let clone; let target = arguments[0] || {}
 		let i = 1; const length = arguments.length; let deep = false
  if (isBoolean(target)) {
    deep = target
    target = arguments[1] || {}
    i = 2
...
export const formatNumber = (num, s = 2) => {
  if (isNumber(num)) {
    ...
    let m = num[0].split('').reverse(); let d
...
export const numberToChinese = (number) => {
  if (isNumber(number)) {
    ...
    var BB = ['', '拾', '佰', '仟', '万', '亿', '圆', '']
    var CC = ['角', '分', '厘']
    var a = number.toString().replace(/(^0*)/g, '').split('.'); var k = 0; var re = ''
...
export const getStrLength = (str) => {
  if (str && str.toString().length) {
    str = str.split('')
    const ch = /^[\u4e00-\u9fa5]+$/
    const reg = /[\-,\/,\|,\$,\+,\%,\&,\',\(,\),\*,\x20-\x2f,\x3a-\x40,\x5b-\x60,\x7b-\x7e,\x80-\xff,\u3000-\u3002,\u300a,\u300b,\u300e-\u3011,\u2014,\u2018,\u2019,\u201c,\u201d,\u2026,\u203b,\u25ce,\uff01-\uff5e,\uffe5]/
